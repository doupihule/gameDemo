...
export default class FogEventTrigger {
	...
	public static checkFrontEvent(eventData: FogEventData) {
		...
		if (!isOver) {
			//@zm  后续换成界面
			WindowManager.OpenUI(WindowCfgs.FogTipUI, {
				"type": FogConst.FOG_VIEW_TYPE_FRONTEVENT,
				"tip": eventData.cfgData.frontEventsTips
			});
		}
		return isOver;
	}
	...
	public static runEvent_13(event: FogEventData, cell: FogInstanceCell) {
		WindowManager.OpenUI(WindowCfgs.FogAnswerUI, {
			event: event,
			cell: cell,
