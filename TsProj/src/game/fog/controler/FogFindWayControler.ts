...
export class FogFindWayControler {
	...
	//目标点
	public targePos: FogInstanceCell;
	//开放列表
	public openList = [];
	//关闭列表
	public closeList = [];
	...
	public constructor(fogControler: FogLogicalControler) {
	...
	findPath(start: FogInstanceCell, end: FogInstanceCell) {
		this.resetPath(end);
		//起点和终点一致 直接返回空列表
		if (start == end) return [];
	...
	getCellItem(cell, pos) {
		...
		var targetCell: FogInstanceCell = this.fogControler.getCellData(id)
