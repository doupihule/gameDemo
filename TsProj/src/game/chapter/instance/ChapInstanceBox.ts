...
export default class ChapInstanceBox extends FogInstanceBasic {
    ...
    onClickItem() {
        ...
        WindowManager.OpenUI(WindowCfgs.ChapterBoxRewardUI, { shareName: ShareTvOrderFunc.SHARELINE_CHAPTERBOX_REWARD, doubleRate: ChapterFunc.ChapterBoxDouble, type: this.type, desc: desc, reward: this.reward, thisObj: this, callBack: this.receiveReward, params: { chapterId: this.fogControler.chapterId, boxId: this.boxId } })
    }
    //记录宝箱的领取状态
    receiveReward() {
        ChapterServer.updateBoxState({ chapterId: this.fogControler.chapterId, boxId: this.boxIndex }, this.freshInfo, this);
