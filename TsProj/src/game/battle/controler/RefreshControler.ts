    ...
    public initWarData() {
        ...
        while (result.length > 0) {
            ...
            for (var j = 0; j < num; j++) {
                ...
                this.controler.createRole(id, { level: level, starLevel: starLevel, equip: equip, passivSkill: enemyInfo.passivSkill }, roleInfo.kind, BattleConst.ROLEGROUP_ENEMY, offestX);
            }
            //重算当前的优先级
    ...
    private onGameOver(rt: number) {
        ...
        if (rt == BattleConst.battleResult_lose) {
            ...
            this.controler.setCallBack(120, () => {
                WindowManager.OpenUI(WindowCfgs.BattleResultUI, { levelId: this.level, isWin: false, controler: this.controler })
            }, this)
        } else {
            if (BattleFunc.curBattleType == BattleConst.BATTLETYPE_NORMAL) {
            ...
            this.controler.setCallBack(120, () => {
                WindowManager.OpenUI(WindowCfgs.BattleResultUI, { levelId: this.level, isWin: true, controler: this.controler })
            }, this)
        }
    }
