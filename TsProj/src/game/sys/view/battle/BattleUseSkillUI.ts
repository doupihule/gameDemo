...
export default class BattleUseSkillUI extends ui.gameui.battle.BattleUseSkillUI {
	...
	onClickUse() {
		if (this.freeType == ShareOrTvManager.TYPE_ADV) {
			StatisticsManager.ins.onEvent(StatisticsManager.SHOWTV_LEVELSKILL_CLICK, {
	...
	private use() {
		if (this.freeType == ShareOrTvManager.TYPE_ADV) {
			StatisticsManager.ins.onEvent(StatisticsManager.SHOWTV_LEVELSKILL_FINISH, {
				id: this.controler.battleData.levelId,
				skillId: this.skillId
			})
		} else if (this.freeType == ShareOrTvManager.TYPE_SHARE) {
			StatisticsManager.ins.onEvent(StatisticsManager.SHARE_LEVELSKILL_FINISH, {
				id: this.controler.battleData.levelId,
				skillId: this.skillId
			})
		}
