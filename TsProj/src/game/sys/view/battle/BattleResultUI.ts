...
export default class BattleResultUI extends ui.gameui.battle.BattleResultUI implements IMessage {
	...
	public setData(data) {
		...
		if (this.isWin) {
			...
			StatisticsManager.ins.onEvent(StatisticsManager.LEVEL_FAIL, {
				levelId: data.levelId,
				time: Client.instance.serverTime - this.controler.startTime
			});
	...
	showFogResultReward() {
		...
		if (this.isWin) {
			...
			StatisticsManager.ins.onEvent(StatisticsManager.SHOWTV_FOG_BATTLE_SHOW, {
	...
	showNormalBattleReward() {
		if (this.isWin) {
			...
			this.battleResultRetio = Number(settlementDouble[this.battleResultCount % settlementDouble.length]);
			StatisticsManager.ins.onEvent(StatisticsManager.LEVEL_VICTORY, {
		...
		BattleServer.battleResult({levelId: this.levelId, doubleRate: 1}, () => {
			if (BattleFunc.curBattleType == BattleConst.BATTLETYPE_WAR) {
				...
				if (!result[0]) {
					...
					WindowManager.OpenUI(WindowCfgs.BattleFullEnergyUI, {
	...
	onReceiveBtnClick() {
		if (this.type == ShareOrTvManager.TYPE_QUICKRECEIVE) {
			...
			if (BattleFunc.curBattleType == BattleConst.BATTLETYPE_WAR && this.isWin) {
				var enemy: FogEventData = FogFunc.enemyCell.eventData;
				StatisticsManager.ins.onEvent(StatisticsManager.SHOWTV_FOG_BATTLE_CLICK, {
					doubleRate: this.battleResultRetio,
					enemyId: this.enemy.enemyData.id
				});
			} else {
	...
	close() {
		...
		if (BattleFunc.curBattleType == BattleConst.BATTLETYPE_NORMAL) {
			...
			if (this.isWin) {
				...
				Message.instance.send(FogEvent.FOGEVENT_REFRESH_CELLEVENT, {
					cell: FogFunc.enemyCell,
					behind: 1,
					type: FogEventTrigger.Event_logical_Enemy
				})
			} else {
