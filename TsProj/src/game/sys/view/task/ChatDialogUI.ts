...
export default class ChatDialogUI extends ui.gameui.task.ChatDialogUI implements IMessage {
	...
	getResultByChat(front, content, info) {
		for (var j = 0; j < front; j++) {
			...
			if (info.answer && info.answer[chatId]) {
				this.infoArr.push({
					type: 2,
					info: TranslateFunc.instance.getTranslate(result.reply[info.answer[chatId] - 1])
				});
			}
		}
	}
	...
	showTxt(index, info, checkNext = false) {
		...
		TimerManager.instance.setTimeout(this.showTxtUI, this, chatInfo.deleyTime, [{
			info: info,
			chatInfo: chatInfo,
			index: index,
			content,
			txt: TranslateFunc.instance.getTranslate(chatInfo.content[lblIndex])
		}])
	...
	onClickWatch() {
		WindowManager.OpenUI(WindowCfgs.ChatDetailUI, {
			cfg: this.cfgData,
			task: this.taskUI,
			chat: this.chatUI,
			dialog: this
	...
	onClickChoose(index) {
		...
		ChatInfoServer.upDateChatInfo({
			role: this.cfgData.role,
			taskId: this.cfgData.id,
			answer: answer
		}, this.continueShowTxt, this, true)
	}
