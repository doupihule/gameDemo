...
export default class WorkInfoItemUI extends ui.gameui.work.WorkInfoItemUI {
	...
	onClickQuickFinish() {
		...
		ShareOrTvManager.instance.shareOrTv(ShareTvOrderFunc.SHARELINE_WORK_FINISH, ShareOrTvManager.TYPE_ADV, {
			id: 1,
			extraData: {}
	...
	receiveCall() {
		...
		WindowManager.OpenUI(WindowCfgs.ChapterBoxRewardUI, {
			viewType: "work",
			reward: this.cfg.reward,
			desc: TranslateFunc.instance.getTranslate("#tid_work_receiveReward"),
			type: ChapterConst.Chapter_boxState_lock,
	...
	checkShowGift() {
		...
		if (gift) {
			WindowManager.OpenUI(WindowCfgs.ChapterBoxDoubleUI, {
				reward: [gift],
				shareName: ShareTvOrderFunc.SHARELINE_WORK_GIFTDOUBLE,
