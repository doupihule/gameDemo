...
export default class AirDropDetailUI extends ui.gameui.main.AirDropDetailUI implements IMessage {
	...
	initView() {
		...
		if (this.freeType == ShareOrTvManager.TYPE_ADV) {
			StatisticsManager.ins.onEvent(StatisticsManager.SHOWTV_FLYBOX_SHOW, {
	...
	onClickReceive() {
		if (this.freeType == ShareOrTvManager.TYPE_ADV) {
			StatisticsManager.ins.onEvent(StatisticsManager.SHOWTV_FLYBOX_CLICK, {
				"rewardType": this.rewardType,
				"rewardCount": this.rewardCount
			});
		}
	...
	successCall() {
		...
		this.close();
		if (this.freeType == ShareOrTvManager.TYPE_ADV) {
			StatisticsManager.ins.onEvent(StatisticsManager.SHOWTV_FLYBOX_FINISH, {
				"rewardType": this.rewardType,
				"rewardCount": this.rewardCount
			});
		} else if (this.freeType == ShareOrTvManager.TYPE_SHARE) {
			StatisticsManager.ins.onEvent(StatisticsManager.SHARE_FLYBOX_FINISH, {
				"rewardType": this.rewardType,
				"rewardCount": this.rewardCount
			});
		}
