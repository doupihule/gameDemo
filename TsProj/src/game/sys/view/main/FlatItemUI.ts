...
export default class FlatItemUI extends ui.gameui.main.FlatItemUI implements IMessage {
    ...
    public unlockNewRole(isnew = true) {
        if (this.getUnlockInfo("right", isnew)) {
            return { item: this.rightRoleClickArea, level: RolesFunc.instance.getUnlockLevel(this.rightRoleData.id), id: this.rightRoleData.id }
        }
        if (this.getUnlockInfo("left",isnew)) {
            return { item: this.leftRoleClickArea, level: RolesFunc.instance.getUnlockLevel(this.leftRoleData.id), id: this.leftRoleData.id }
        }
        return null
    }
