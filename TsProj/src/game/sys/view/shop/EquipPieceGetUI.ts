...
export default class EquipPieceGetUI extends ui.gameui.shop.EquipPieceGetUI implements IMessage {
	...
	onClickAd() {
		...
		ShareOrTvManager.instance.shareOrTv(ShareTvOrderFunc.SHARELINE_EQUIP_GET, ShareOrTvManager.TYPE_ADV, {
			id: 1,
			extraData: {}
		}, this.successCallBack, this.failCall, this)

	}

	successCallBack() {
		if (this.type == ShareOrTvManager.TYPE_ADV) {
			StatisticsManager.ins.onEvent(StatisticsManager.SHOWTV_VIDEOPIECE_FINISH, {
				count: this.videoCount + 1,
				reward: this.adReward
			})
		} else {
			StatisticsManager.ins.onEvent(StatisticsManager.SHARE_VIDEOPIECE_FINISH, {
				count: this.videoCount + 1,
				reward: this.adReward
			})
		}
