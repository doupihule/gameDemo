...
export default class FogServer {
    ...
    static saveFogReward(data, callBack = null, thisObj = null, isAsyc = true) {
        ...
        if (totalCoin != "0") {
            reward[DataResourceConst.COIN] = BigNumUtils.sum(FogModel.instance.getFogRewardNum(DataResourceConst.COIN), totalCoin);
        }
        if (totalGold != "0") {
            reward[DataResourceConst.GOLD] = BigNumUtils.sum(FogModel.instance.getFogRewardNum(DataResourceConst.GOLD), totalGold);
        }
        if (totalFogCoin != 0) {
            reward[DataResourceConst.FOGCOIN] = FogModel.instance.getFogRewardNum(DataResourceConst.FOGCOIN) + totalFogCoin;
        }
        if (totalComp != 0) {
            reward[DataResourceConst.COMP] = FogModel.instance.getFogRewardNum(DataResourceConst.COMP) + totalComp;
        }
        if (totalAct != 0) {
            reward[DataResourceConst.ACT] = FogModel.instance.getFogRewardNum(DataResourceConst.ACT) + totalAct;
        }
        ...
        if (Object.keys(pieceTab).length != 0) {
            ...
            for (var pieceId in pieceTab) {
                pieceData[pieceId] = FogModel.instance.getFogRewardNumById(DataResourceConst.PIECE, pieceId) + pieceTab[pieceId];
            }
            reward[DataResourceConst.PIECE] = pieceData;
        }
        if (Object.keys(propTab).length != 0) {
            ...
            for (var itemId in propTab) {
                ...
                num = FogModel.instance.getFogRewardNumById(DataResourceConst.FOGITEM, itemId) + propTab[itemId];
            ...
            reward[DataResourceConst.FOGITEM] = itemData;
        }
