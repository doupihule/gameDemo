...
import SupplyBoxFunc from "../func/SupplyBoxFunc";
import RecycleFunc from "../func/RecycleFunc";
...
export default class UserExtServer {
   ...
   */
    ...
    static setAutoOpenAirDrop(data = null, callBack = null, thisObj = null) {
        ...
        var count = CountsModel.instance.getCountsById(CountsModel.autoOpenAirDrop);
        if (count == 0) {
            countData["expireTime"] = GameUtils.getNextRefreshTByTime(4);
        }
        countData["id"] = CountsModel.autoOpenAirDrop;
        countData["count"] = count + 1;
        upData["counts"] = {
            [CountsModel.autoOpenAirDrop]: countData
        }
