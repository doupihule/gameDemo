...
export default class AlertNewLocalUI extends Laya.View {
	public static res: string = null;


	static _instance: AlertNewLocalUI = null;
	static get instance(): AlertNewLocalUI {
		if (!this._instance) {
	...
	constructor() {
		...
		this.height = ScreenAdapterTools.height;
	...
	private _sureRt: any = {confirm: true}
	private _cancleRt: any = {cancel: true}

	//type 1是确认 2是取消
	private touchHandler(type: number = 1): void {
		this.close();
		LogsManager.echo("____ontouchHandler,", this._cacheDataArr)
	...
	public close() {
		...
		this.removeSelf();
		if (WindowManager.highLayer.numChildren == 0) {
			WindowManager.highLayer.mouseEnabled = false;
		}
		PoolTools.cacheItem("AlertNewLocalUI", this, "sys");
	}
