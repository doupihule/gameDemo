            ...
            if (this.getWX().navigateToMiniProgram) {
                this.getWX().navigateToMiniProgram({
                    ...
                    success: (res) => {
                        ...
                        StatisticsManager.ins.onEvent(StatisticsCommonConst.JUMP_TO_OTHER_GAME_SUCCESS, { gameName: data.jumpData.GameName, from: data.extraData && data.extraData.from, toAppId: data.appId, position:position })
            ...
            api.onAntiAddiction(function ({state, msg}) {
                if(!mythis.antiAddtionInfo)
                {
                    ...
                    TimerManager.instance.setTimeout(()=>{
                        WindowManager.setPopupTip(1, TranslateFunc.instance.getTranslate("#tid_anti_indulge2","TranslateGlobal"), ()=>{mythis.getWX().exit()}, mythis);
                    },mythis,3000);
                }
