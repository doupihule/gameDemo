...
export default class GamePlatform implements IMessage {
    ...
    */
     ...
     * @param thisObject
     */
    getWX(): any { return ControlConst.emptyTable }
    registerAd() { }
     ...
     * 退出游戏
     */
    loginOut(): void { Global.isGameDestory = true }
     ...
     */
    vibrate(isLong: boolean = false, callBack: Function = null, thisObject: any = null) { callBack && callBack.call(thisObject, true); }
    /**执行count次的短震 */
    ...
    initCityName() {
        ...
        if (this.cityName)  {
            ...
            HttpMessage.instance.sendOtherHttpRequest(url, null, this.initCityNameCallBack, this, "get", { errorCall: this.initCityNameErrorCallBack, contentType: "application/json" })
        }
        return;
    }
    ...
    protected onLoseFocus() {
        ...
        // 发送阿里云打点日志
        StatisticsManager.ins.onEvent(StatisticsCommonConst.ON_HIDE, { 'onlineTime': this.hideT - this.showT, 'hideView':GameUtils.decryptStr(WindowManager.getCurrentWindowName())});
    }
