...
export default class TTGamePlatform extends WXGamePlatform {
    ...
    shareVideo(callBack: Function = null, thisObj = null) {
        StatisticsManager.ins.onEvent(StatisticsCommonConst.SHARE_VIDEO_CLICK, { videoPath: this._videoPath, type: this._recordType });
    ...
    showMoreGamesModal(data) {
        ...
        if (!this._hasInitMoreGame) {
            ...
            tt.onNavigateToMiniProgram(function (res) {
                if (data) {
                    StatisticsManager.ins.onEvent(StatisticsCommonConst.JUMP_TO_OTHER_GAME_SUCCESS, { gameName: data.jumpData && data.jumpData.GameName, from: data.extraData && data.extraData.from, toAppId: data && data.appId, position: data.jumpData.Position });
                }
                else {
