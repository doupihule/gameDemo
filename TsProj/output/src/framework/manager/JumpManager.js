...
const ResultJumpUI_1 = require("../../game/sys/view/jump/ResultJumpUI");
...
const JumpCommonConst_1 = require("../consts/JumpCommonConst");
const ResultJumpDoubleUI_1 = require("../../game/sys/view/jump/ResultJumpDoubleUI");
...
class JumpManager {
    ...
    static sendKariquReq(callBack, thisObject, callParams = null, mutualType = null) {
        ...
        HttpMessage_1.default.instance.sendOtherHttpRequest(url, params, this.onKariquSuccess, this, "post", null, 1, { info: kariquInfo, list: kariquList });
    }
     ...
     * }
     */
    static addJumpUI(parent, extraData = {}, param = {}) {
        if (!param.uiType) {
            param.uiType = JumpCommonConst_1.default.JUMP_LIST_SOFT_UI;
        }
        if (!this.checkShow())
            return;
        var jumpData;
        extraData.type = JumpConst_1.default.JUMP_TYPE_JIESUAN;
        if (param.uiType != JumpCommonConst_1.default.JUMP_LIST_SOLID_UI) {
            jumpData = this.getMokaDataByType(JumpConst_1.default.JUMP_TYPE_JIESUAN);
        }
        else {
        ...
        if (jumpData.length <= 0)
            return;
        var cfgs = WindowManager_1.default.getWindowCfgs(WindowCommonCfgs_1.default.ResultJumpView);
        var res = cfgs.group;
        var subpackage = cfgs.subPackage;
        var resAll = [];
        LogsManager_1.default.echo("extraData.from---------------", extraData.from);
        var tempFunc = () => {
            this.initJumpUI(parent, jumpData, extraData, param);
        };
        if (extraData.isDouble) {
            var cfgs = WindowManager_1.default.getWindowCfgs(WindowCommonCfgs_1.default.ResultJumpDoubleView);
            res = res.concat(cfgs.group);
        }
        LoadManager_1.LoadManager.instance.loadPacgeAndRes(subpackage, res, Laya.Handler.create(this, tempFunc));
    }
    /**获取静互推刷新数据 */
    ...
    static initJumpUI(parent, jumpData, extraData, param) {
        ...
        LogsManager_1.default.echo("extraData.from1---------------", from);
        ...
        jumpUI.y = (extraData.posY != undefined) ? extraData.posY : (120 + ScreenAdapterTools_1.default.toolBarWidth);
    }
    /**移除互推UI */
    ...
    static getJumpUIIns(data, extraData, param) {
        LogsManager_1.default.echo("extraData.from2-------------", extraData.from);
        if (extraData.isDouble) {
            ...
            LogsManager_1.default.echo("yrc new ResultJumpDoubleUI");
            var ui2 = new ResultJumpDoubleUI_1.default(data, 90, false, extraData);
            ...
            if (this.jumpUIs.length > 0) {
                ...
                LogsManager_1.default.echo("yrc new ResultJumpUI");
                var ui = new ResultJumpUI_1.default(data, extraData, param);
                ui.initData(data);
                return ui;
            }
        }
    ...
    static addMainJump(parent, jumpPos, posArr = [], isNeedSign = false, isShowBg = false, mutualType = null, isShowName = false, txtSize = 15, txtcolor = "ffffff", isShowLableBg = false, signWidth = 0, signHeight = 0, bgextraWidth = 0, isNeedRemove = true, isNeedOpenJumpUI = false) {
        ...
        for (var i = 0; i < posArr.length; i++) {
            ...
            if (jumpData) {
                ...
                var mainJumpItem = this.createJumpItem(jumpData, posItem.width, posItem.width, extraData, isNeedSign, JumpConst_1.default.JUMP_ANI_STYLE_1, isShowName, txtSize, txtcolor, isShowLableBg, signWidth, signHeight, isShowBg, bgextraWidth);
    ...
    static startMainJumpTween(mainJumpItem) {
        mainJumpItem["__tempTimeCode"] = TimerManager_1.default.instance.setTimeout(this.scaleShake, this, 3000, mainJumpItem);
    }
    /**
     * 暂停图标动画
     */
    static stopMainJumpTween(mainJumpItem) {
        ...
        if (mainJumpItem["__tempTimeCode"]) {
            TimerManager_1.default.instance.remove(mainJumpItem["__tempTimeCode"]);
    ...
    static refreshMainJumpImg(mainJumpItem, restart = true) {
        //如果当前是显示在舞台的才执行
        if (mainJumpItem.stage) {
            var img = mainJumpItem.getChildByName("imageIcon");
            var lastIndex = 0;
            var jumpData;
            if (KariqiShareManager_1.default.checkIsKariquChannel()) {
                ...
                if (this.curGroup.indexOf(lastIndex) == -1) {
                    ...
                    LogsManager_1.default.echo("zm.repeat--------------", lastIndex);
                }
                jumpData = this.getRandomJump(this.curGroup);
            }
            ...
            var txt = mainJumpItem.getChildByName("imageTxt");
            if (txt) {
                txt.text = jumpData.GameName;
            }
            img.skin = jumpData.Icon;
            var lastButtonUtil = img["__lastButtonUtils"];
            if (lastButtonUtil) {
                //重置按钮的回调参数
    ...
    static createJumpItem(itemData, itemWidth, itemHeight, extraData, needRedPoint = null, aniStyle = 0, isShowName = true, txtSize = 15, txtcolor = "#ffffff", isShowLableBg = false, signWidth = 0, signHeight = 0, isShowBg = false, bgextraWidth = 0, txtOffestY = 0) {
        ...
        var redOffset = 3;
        var startY = 7;
        var itemBox = new Laya.Image();
        itemBox.width = itemWidth + bgextraWidth * 2;
        itemBox.height = itemHeight + bgextraWidth * 2;
        //icon背景        
        if (isShowBg) {
            var itemBg = new Laya.Image(ResourceConst_1.default.JUMP_ITEMBG);
        ...
        if (needRedPoint) {
            if (this.jumpChannel == JumpConst_1.default.JUMP_CHANNEL_ZHISE || KariqiShareManager_1.default.checkIsKariquChannel()) {
                var sign = new Laya.Image(needRedPoint);
                ...
                var redImg = new Laya.Image(ResourceConst_1.default.COMMON_REDPOINT);
        ...
        if (isShowLableBg) {
            if (this.jumpChannel == JumpConst_1.default.JUMP_CHANNEL_ZHISE) {
                var labelBg = new Laya.Image(this.getZhiseLabelBg());
            ...
            else if (this.jumpChannel == JumpConst_1.default.JUMP_CHANNEL_KARIQU) {
                var labelBg = new Laya.Image(this.getZhiseLabelBg(3));
                itemBox.addChild(labelBg);
                labelBg.width = itemWidth + bgextraWidth * 2;
                labelBg.height = (itemWidth + bgextraWidth * 2) / 3.7;
                labelBg.x = 0;
        ...
        if (isShowName) {
            var nameTxt = new Laya.Label(itemData.GameName);
        ...
        var btnUtil = new ButtonUtils_1.ButtonUtils(imgItem, this.onClickJumpItem, this, null, null, sendData);
        // 防误触
    ...
    //销毁一个跳转的item
    static destoreJumpItem(itemBox) {
        // 从parent移除
        if (itemBox.parent)
            itemBox.parent.removeChild(itemBox);
        // 销毁按钮
        var imageIcon = itemBox.getChildByName("imageIcon");
