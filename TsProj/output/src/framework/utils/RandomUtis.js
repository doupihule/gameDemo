...
class RandomUtis {
    ...
    static getOneGroupIndex(restrict, nums, index = 0) {
        if (!nums) {
            nums = restrict;
        }
        var resultArr = [];
        ...
        for (var i = restrict - 1; i >= restrict - nums; i--) {
            var randomInt = this.getOneRandomInt(i, 0, index);
            var value_1 = tempArr[randomInt];
            var value_2 = tempArr[i];
    ...
    static getOneRandomInt(restrict, min = 0, index = 0) {
    ...
    /**
      ...
      * nums 获取的数的数量
      */
    static getOneIndexByWeight(arr, index = 0) {
        var totalValue = 0;
        var tempArr = [];
    ...
    //获取一系列权重数序号
    static getIndexArrByWeight(arr, nums = 1, index = 0) {
        var randomArr = [];
        var cloneArr = TableUtils_1.default.copyOneArr(arr);
        var resultArr = [];
        var num = arr.length;
        if (num < nums) {
            LogsManager_1.default.errorTag(null, 'getIndexArrByWeight  ````  arr长度小于nums！！');
            nums = num;
        }
    ...
    //随机一个数组
    static randomOneGroup(arr, index = 0) {
        var resultArr = [];
        this.randomOneGroupToOut(arr, resultArr, index);
        return resultArr;
    }
    //随机一个数组 输出目标数组 原数组和输出数组不能是同一个.否则会出错
    static randomOneGroupToOut(arr, resultArr, index = 0) {
        var nums = arr.length;
        var indexArr = this.getOneGroupIndex(nums, nums, index);
        if (!resultArr) {
            resultArr = [];
        }
        for (var i = 0; i < nums; i++) {
...
exports.default = RandomUtis;
RandomUtis._yinziToIndexDic = {};
RandomUtis.RANDOM_MAX = 2147483647;
RandomUtis.MIDDLEVALUE = 127773;
RandomUtis.OFFSETVALUE = 2836;
RandomUtis.MODVALUE = Math.floor(RandomUtis.RANDOM_MAX / RandomUtis.MIDDLEVALUE);
...
//# sourceMappingURL=RandomUtis.js.map