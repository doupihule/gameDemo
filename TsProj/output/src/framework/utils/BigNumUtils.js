...
class BigNumUtils {
    ...
    static sum(a, b) {
        ...
        if (indexa != -1) {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算加法a出现小数", a);
            a = a.substr(0, indexa);
        }
        if (indexb != -1) {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算加法b出现小数", b);
            b = b.substr(0, indexb);
        }
        //结果
        var result = "";
    ...
    static substract(a, b) {
        ...
        var indexb = b.indexOf(".");
        if (indexa != -1) {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算减法a出现小数", a);
            a = a.substr(0, indexa);
        }
        if (indexb != -1) {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算减法b出现小数", b);
            b = b.substr(0, indexb);
        }
        //结果
        var result = "";
    ...
    static muitlfy(a, b) {
        ...
        var indexb = b.indexOf(".");
        if (indexa != -1) {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算乘法a出现小数", a);
            a = a.substr(0, indexa);
        }
        if (indexb != -1) {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算乘法b出现小数", b);
            b = b.substr(0, indexb);
        }
        //判断符号
        var waya = a.slice(0, 1) == "-";
    ...
    static devide(a, b, round = 0) {
        //转换成字符串，兼容int类型
        a = a + "";
        b = b + "";
        //判零
        if (b == "0") {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算除法除数为0", b);
            return 0;
        }
        //出现小数的处理：打error，并强制取整
        var indexa = a.indexOf(".");
        var indexb = b.indexOf(".");
        if (indexa != -1) {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算除法a出现小数", a);
            a = a.substr(0, indexa);
        }
        if (indexb != -1) {
            LogsManager_1.default.errorTag("BigNumUtils", "大数运算除法b出现小数", b);
            b = b.substr(0, indexb);
            if (b == "0") {
                return 0;
            }
    ...
    static compare(a, b, equal = false) {
        //转换成字符串，兼容int类型
        a = a + "";
        b = b + "";
        if (equal && a == b) {
