...
class InstanceMove extends InstanceBasic_1.default {
    constructor(controler) {
        ...
        //地面坐标
        this.landPos = BattleFunc_1.default.defaultRoleYpos;
    ...
    movePos() {
        ...
        if (this.speed.y < 0 && this.pos.y <= this.landPos && this._myState == BattleConst_1.default.state_jump && this.gravityAble) {
            this.onHitLand();
        }
    }
    ...
    moveToOnePoint(x, y, z = 0, spd = 0, callFunc = null, thisObj = null, callParams = null, expandParams = null, movePointType = 1) {
        if (!this._moveParams) {
            this._moveParams = new InstanceMoveEntity_1.default(new Laya.Vector3(x, y, z));
        }
        ...
        if (this._moveParams.totalFrame > 0 && spd == 0) {
            ...
            spd = spd || BattleFunc_1.default.moveSpeed;
        }
    ...
    countMoveSpeed() {
        ...
        //计算目标向量差值
        Laya.Vector3.subtract(this._moveParams.target, this.pos, temp);
        var speed = this._moveParams.spd;
        //向量归一
    ...
    moveToGroupPoints(pointArr, speed = 0, callFunc = null, thisObj = null, expandParams, loopParams = null) {
    ...
    //朝目标运动
    moveToTargetRole(spd, targetRole) {
     ...
     *   6   朝向(-1,永远朝左,0 跟随目标的朝向旋转,1永远朝右);
         ...
         11  特效总帧数;
    */
    createEffByParams(effectParamsArr, isFollow = false, isLoop = false, expandParams = null, aniSpeed = 1) {
        if (!effectParamsArr || effectParamsArr.length == 0) {
            return;
        }
        if (!this.controler) {
            LogsManager_1.default.warn("已经放入缓存了不应该走到这里来");
            return;
        }
    ...
    //创建单个特效
    createOneEffByParam(params, isFollow = false, isLoop = false, expandParams = null, aniSpeed = 1) {
        ...
        if (delayTime > 0) {
            this.controler.setCallBack(BattleFunc_1.default.instance.turnMinisecondToframe(delayTime), this.delayCreateEffByParams, this, params);
        }
        else {
    ...
    //延迟创建特效片
    delayCreateEffByParams(params) {
        var isFollow = params.isFollow;
        var isLoop = params.isLoop;
        var offsetType = Number(params[2]);
        var offsetX = Number(params[3]);
        var offsetY = Number(params[4]);
        var effectName = params[0];
        var aniIndex = Number(params[1]);
        var aniSpeed = params.aniSpeed || 1;
        if (offsetType == 2) {
            if (!this.cfgData) {
                LogsManager_1.default.errorTag("cgfserror", "没有配置size,rid:", this.dataId, "类型:", this["lifeType"]);
            }
        ...
        var ani = this.createEfect(effectName, aniIndex - 1, isLoop, offsetX, offsetY, 0, isFollow, layerIndex, Number(params[6]), frame, aniSpeed);
        var sysFrame = Number(params[8]);
        if (sysFrame && sysFrame > 0 && params.length > 10) {
            ani.playSpecialSysAction(aniIndex - 1, sysFrame, Number(params[9]), Number(params[10]), Number(params[11]) * BattleFunc_1.default.battleViewFrameScale, aniSpeed);
        }
        //设置特效扩展参数
        ...
        if ((params.expandParams && params.expandParams.follow) || isFollow || layerIndex == 5) {
            if (params.expandParams && params.expandParams.type == SkillExpandTrigger_1.default.EXPAND_TYLE_LINEEFFECT)
                return;
    ...
    //清除一组特效 
    clearEffByParams(effectParams) {
        if (!effectParams || effectParams.length == 0) {
            return;
        }
        for (var i = 0; i < effectParams.length; i++) {
            //特效缓存是  特效名字+特效动作序号
            this.clearEffByName(PoolCode_1.default.POOL_EFFECT + effectParams[i][0] + Number(effectParams[i][1] - 1));
        }
    }
    //清除一个特效
    clearEffByName(name, clearAll = false) {
        for (var i = this._followEffGroup.length - 1; i >= 0; i--) {
            var eff = this._followEffGroup[i];
            if (eff.cacheId == name) {
