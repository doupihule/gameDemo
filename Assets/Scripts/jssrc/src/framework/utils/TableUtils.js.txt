...
class TableUtils {
    ...
    //深度合并
    static deepMerge(receiveObj, fromObj) {
        if (!fromObj) {
            return;
        }
        if (!receiveObj) {
    ...
    //浅拷贝一个数组
    static copyOneArr(arr, outArr = null) {
        if (!outArr) {
            outArr = [];
        }
        else {
    ...
    //找一个表里面的空字段
    static findNullKey(fromTb, toTb) {
        if (!fromTb) {
            return toTb;
        }
        for (var i in fromTb) {
    ...
    static turnCsvArrToGameArr(fromArr, transtoNumber = false) {
        var toArr = [];
        if (fromArr) {
    ...
    static turnCsvStrTogGameArr(fromStr, transtoNumber = false) {
        //如果是以分号结尾 那么移出最后一个字符
        var strLen = fromStr.length;
    ...
    //清空一个table表
    static clearOneTable(targetTb) {
        for (var i in targetTb) {
            delete targetTb[i];
        }
    }
    //安全json解析
    static safelyJsonStringfy(obj) {
        var rt = '';
        try {
